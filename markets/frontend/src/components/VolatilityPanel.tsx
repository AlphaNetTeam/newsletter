...
import type { VolatilityData } from "../api/types";
...
/** The "X at Y% annualised volatility is ..." blurb. Handles
 * vsBtcMultiple/sp500Current being null (e.g. BTC's own candle cache
 * isn't warm yet) without ever interpolating "undefined" into the copy.
 */
function volatilityBlurb(symbol: string, series: NonNullable<VolatilityData["series"]>): string {
...
export default function VolatilityPanel({ volatility, symbol }: Props) {
  const series = volatility?.series;
  const points = series?.points ?? [];
  const chartData = points.map((p) => ({ t: p.t, vol: p.vol * 100 }));

  return (
    <div>
      <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 4 }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Volatility and risk profile</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 20 }}>
        ...
        >
          <div style={{ height: 220, opacity: points.length ? 1 : 0.4 }}>
          ...
          {series && (
            <div style={{ display: "flex", gap: 32, marginTop: 12, fontSize: 13 }}>
              ...
              {series.vsBtcMultiple !== null && series.vsBtcMultiple !== undefined && (
                <Stat label="VS BTC" value={`${series.vsBtcMultiple.toFixed(1)}×`} />
              )}
            </div>
          )}
        </div>
        ...
        <Panel title="WORST DRAWDOWN, STRATEGY VS HOLDING">
          {(volatility?.drawdownCompare ?? []).length === 0
            ? Array.from({ length: 5 }).map((_, i) => (
                <div key={i} style={{ height: 16, background: "rgba(255,255,255,0.04)", borderRadius: 4 }} />
              ))
            : (() => {
                const entries = volatility!.drawdownCompare;
                const maxVal = Math.max(...entries.map((e) => e.maxDrawdown), 0.01);
